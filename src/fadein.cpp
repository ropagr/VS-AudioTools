...
constexpr const char* FuncName = "FadeIn";
...
static void VS_CC fadeinCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi)
{
    // clip:anode
    int err = 0;
    ...
    // start_second:float:opt
    int64_t startSample = vsmap::getOptSamples("start_sample", "start_second", in, out, vsapi, 0, audioInfo->sampleRate);
    ...
    common::Transition* trans = common::newTransition(optFadeType.value(), 0, 0, static_cast<double>(fadeSamples) - 1, 1);
    ...
    Fade* data = new Fade(audio, audioInfo, startSample, fadeSamples, optChannels.value(), trans, optOverflowMode.value(), optOverflowLog.value(), FuncName);
